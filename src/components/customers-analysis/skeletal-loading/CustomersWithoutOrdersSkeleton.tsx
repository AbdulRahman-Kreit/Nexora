"use client";
import React from 'react';

export default function CustomersWithoutOrdersSkeleton() {
    const heights = ['60%', '50%', '55%', '35%', '60%', '85%', '65%', '58%', '40%', '30%'];

    return (
        <div 
            className={`
                bg-main-gradient ml-1 p-6 h-96 border-l-3 border-[#4a7fce] 
                animate-pulse relative overflow-hidden transition-all duration-500
            `}
            aria-hidden="true"
        >
            <div className="h-4 w-64 bg-(--field-bg-color) opacity-50 rounded-sm mb-12"></div>

            <div className="flex items-end justify-between h-56 px-2 gap-3">
                {[...Array(10)].map((_, i) => (
                    <div key={i} className="flex-1 flex flex-col items-center gap-4">
                        <div className="h-3 w-6 bg-(--field-bg-color) opacity-30 rounded-xs"></div>
                        
                        <div 
                            className="w-full max-w-[25px] bg-(--field-bg-color) opacity-40 rounded-t-[5px]" 
                            style={{ height: heights[i] }}
                        ></div>
                        
                        <div className="h-3 w-10 bg-(--field-bg-color) opacity-30 rounded-sm mt-2"></div>
                    </div>
                ))}
            </div>

            <div className="absolute inset-0 -translate-x-full animate-[shimmer_2s_infinite] bg-linear-to-r from-transparent via-(--main-text-color) opacity-5 to-transparent"></div>
        </div>
    );
}